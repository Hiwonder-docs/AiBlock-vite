...
- Complete Program
  ...
  1. When executing the main program after powering on, it first initializes the ultrasonic sensor on port A, then enters a continuous repeat loop to poll distance data in real time:
     ...
     * When obstacle distance is under 10 cm, servo S1 rotates without delay to 90° at 40% speed, then waits 1 s.

     * When obstacle distance is greater than or equal to 10 cm, servo S1 rotates without delay to 0° at 40% speed, then waits 1 s.

  2. Independent button control logic allows manually overriding the servo position:
     * Short-pressing the left button rotates servo S1 without delay to 0° at 40% speed.
     
     * Short-pressing the right button rotates servo S1 without delay to 90° at 40% speed.
